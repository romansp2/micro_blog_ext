class TermTaxonomiesController < ApplicationController
  before_action :set_term_taxonomy, only: %i[show edit update destroy]
  ...
  def update
    respond_to do |format|
      if @term_taxonomy.update(term_taxonomy_params)
        ...
        format.json { render :show, status: :ok, location: @term_taxonomy }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to term_taxonomies_url, notice: 'Term taxonomy was successfully destroyed.' }
  ...
  def term_taxonomy_params
    params.require(:term_taxonomy).permit(:term_id, :taxonomy, :description, :parent, :count)
  end
end
